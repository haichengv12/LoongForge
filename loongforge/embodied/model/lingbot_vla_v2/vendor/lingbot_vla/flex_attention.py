...
# Defined outside the version guard so ``_get_flex_attention`` never raises
# NameError on a torch build without flex attention.
...
if Version(torch.__version__) > Version("2.5.0"):
    ...
    # Calling `flex_attention` eagerly runs the op-by-op reference path, which
    # materializes the [B, H, Q, KV] scores and costs host time that scales with
    # shape (measured B=10/H=32/d=128: 8.8 / 30.7 / 115.5 ms per call at seq
    # 1024 / 2048 / 4096). Compiling once at module level lowers it to the Triton
    # kernel and a flat ~0.11 ms of host time per call.
    # Set ``model.flex_compile: false`` to fall back to the eager path for A/B runs.
    _flex_attention_eager = flex_attention
    _flex_attention_compiled = torch.compile(flex_attention, dynamic=False)

# Module-level rather than threaded through every attention call: the forward is
# vendored code several frames below any config, and the choice is fixed for the run.
_FLEX_COMPILE = True


def set_flex_compile(enabled: bool) -> None:
    """Select the compiled or eager path. Call once, before the first forward."""
    global _FLEX_COMPILE
    _FLEX_COMPILE = bool(enabled)


def _get_flex_attention():
    """Vendor interface."""
    fn = _flex_attention_eager if not _FLEX_COMPILE else _flex_attention_compiled
    if fn is None:
        raise RuntimeError(
            "flex attention requires torch > 2.5.0, "
...
def flex_attention_with_block_mask(
    ...
    attn_output, _ = _get_flex_attention()(
        query_states,
        key_states,
        value_states,
        block_mask=block_mask,
