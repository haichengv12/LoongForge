...
from loongforge.embodied.model.lingbot_vla_v2.vendor.models_common.loader import LingBotVLAWeightLoader
from loongforge.embodied.model.lingbot_vla_v2.vendor.ops.triton_moe_loss import triton_sequence_wise_balance_loss
...
class QwenvlWithExpertV2Model(PreTrainedModel):
    ...
    def build_prefix_position_ids(self, input_ids, attention_mask,
        ...
        # transformers 4.x: no mm_token_type_ids; transformers 5.x: required.
        """Vendor interface."""
        import inspect
        ...
        try:
            sig = inspect.signature(self.qwenvl.model.get_rope_index)
            if "mm_token_type_ids" in sig.parameters:
                mm_token_type_ids = torch.zeros_like(input_ids, dtype=torch.int)
                image_token_id = getattr(self.qwenvl.config, "image_token_id", None)
                video_token_id = getattr(self.qwenvl.config, "video_token_id", None)
            ...
            pass
        position_ids, _ = self.qwenvl.model.get_rope_index(**rope_kwargs)
        return position_ids
    ...
    def forward(
        ...
        if not (0 <= layer_start < layer_end <= num_layers):
            raise ValueError(
                f"invalid interleaved layer range [{layer_start}, {layer_end}) for {num_layers} layers"
            )
        router_logits_list = []
        ...
        if layer_end != num_layers:
            return inputs_embeds, past_key_values, router_logits_list
...
class FlowMatchingV2(FlowMatchingV1):
    ...
    def forward(
        ...
        dtype = state.dtype
        device = state.device
        fm_gen = self._fm_generator(device)
        if noise is None:
            noise = torch.randn(actions.shape, device=device, dtype=dtype, generator=fm_gen)
        if time is None:
            time = self.sample_time(actions.size(0), device, generator=fm_gen).to(dtype)
        ...
        if self.config.align_params != {}:
            ...
            if pending is not None:
                # Join the teacher stream here, not earlier: everything above this
                # point is independent of the targets, and that window is what hides
                # the teacher's cost.
                self._pending_teacher = None
                targets = pending.result()
                depth_targets = targets["depth_targets"]
                future_depth_targets = targets["future_depth_targets"]
                future_video_targets = targets.get("future_video_targets")
                future_video_cls_targets = targets.get("future_video_cls_targets")
                future_video_current_patch = targets.get("future_video_current_patch")
        ...
        if router_logits_list:
            ...
            with torch.no_grad():
                for i, logits in enumerate(router_logits_list):
                    ...
                    action_layer = self.qwenvl_with_expert.qwen_expert.model.layers[layer_id]
                    moe_block = action_layer.mlp if action_layer is not None else None
                    if moe_block is not None and hasattr(moe_block, "last_tokens_per_expert"):
                        if not moe_block.last_tokens_per_expert_valid:
                            ...
                            # The flag is the host-side equivalent of the old
                            # ``counts.sum() == 0`` test, which forced a device sync here
...
class LingbotVlaV2Policy(PreTrainedModel):
    ...
    def set_pending_teacher(self, handle):
        """Hand an in-flight teacher result to the module that consumes it.

        The trainer submits the teachers before the forward and the auxiliary heads
        resolve them, so the handle has to cross from the recipe down to
        FlowMatchingV2 without being routed through the forward kwargs. Unwraps
        DDP/FSDP-style wrappers and fails loudly rather than silently dropping the
        handle, which would leave the depth head with no targets.
        """
        target = self.model
        for _ in range(8):
            if isinstance(target, FlowMatchingV2):
                target._pending_teacher = handle
                return
            target = getattr(target, "module", None)
            if target is None:
                break
        raise RuntimeError(
            "cannot locate FlowMatchingV2 to attach the pending teacher handle"
        )
    ...
    def forward_layer_range(self, *args, layer_start: int, layer_end: int, **kwargs):
        """Run a contiguous range of paired VLM/action layers.

        This is the explicit stage boundary used by the experimental Pipeline
        backend. The default full-model forward remains unchanged.
        """
