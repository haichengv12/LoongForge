...
# Process-global FM seed. FSDP/torch.compile may hide module attributes; a
# process-local int is the most reliable way to align noise/time across trainers.
_FM_STEP_SEED = None
...
def sample_beta(alpha, beta, bsize, device, generator=None):
    # Optional generator makes flow-matching time sampling reproducible across
    # trainers that differ in CUDA-RNG consumption before the model forward.
    """Vendor interface."""
...
def block_suffix_to_fv_(att_2d_masks, suffix_row_start, prefix_len,
    ...
    if drop_mask is None:
        ...
        keep = drop_mask.logical_not().view(-1, 1, 1)
