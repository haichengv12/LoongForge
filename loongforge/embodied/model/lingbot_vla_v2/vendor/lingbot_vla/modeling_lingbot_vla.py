...
from loongforge.embodied.model.lingbot_vla_v2.vendor.vision_models.align_heads.depth_head import TaskTokenDepthHead
...
class QwenvlWithExpertModel(PreTrainedModel):
    ...
    def get_image_features(
        ...
        Args:
            ...
            precompute_grid_thw (`bool`): If True, compute and cache
                rotary_pos_emb/window_index/cu_seqlens on first call.
        """
    ...
    def forward(
        ...
        Returns:
            ...
            past_key_values (Optional[Union[List[torch.FloatTensor], Cache]]):
                Optional kv cache.
        """
        models = [self.qwenvl.model, self.qwen_expert.model]  # Qwen2_5_VLTextModel, Qwen2Model
            ...
            if self.config.attention_implementation == "flex_cached":
                if layer_idx == 0:
                    ...
                    _full_block_mask = build_block_mask(
                        attention_mask,
                        self.qwenvl.config.num_attention_heads,
                        _full_len,
                        _full_len,
                    )
...
class LingbotVlaPolicy(PreTrainedModel):
    ...
    _no_split_modules = [
        ...
        "FixQwen2RMSNorm",
        "FixAdaRMSNorm",
    ]  # NOTE: nested modules of Qwen2DecoderLayer need not be listed.
...
class FlowMatching(nn.Module):
    ...
    def sample_time(self, bsize, device, generator=None):
        time_beta = sample_beta(1.5, 1.0, bsize, device, generator=generator)
        time = time_beta * 0.999 + 0.001
        return time.to(dtype=torch.float32, device=device)

    def _fm_generator(self, device):
        """Per-step CUDA generator for flow-matching noise/time.

        Prefer process-global seed (set_fm_step_seed) so FSDP/torch.compile
        wrappers cannot hide the attribute; fall back to ``self._fm_step_seed``.
        """
        step_seed = get_fm_step_seed()
        if step_seed is None:
            step_seed = getattr(self, "_fm_step_seed", None)
        if step_seed is None:
            return None
    ...
    def embed_suffix(self, state, noisy_actions, timestep):
        ...
        action_emb = self._fp32_linear(self.action_in_proj, noisy_actions) if _fp32 else self.action_in_proj(
            noisy_actions) # torch.Size([1, state_bs*50, 1024])
    ...
    def forward(
        ...
        if seq_wise_loss_coeff > 0 and router_logits_list:
            from loongforge.embodied.model.lingbot_vla_v2.vendor.ops.triton_moe_loss import (
    ...
    def depth_emb_forward(self, hidden_states, depth_targets=None, img_masks=None, future_depth_targets=None):
        ...
        depth_preds = self.depth_align_embs.repeat(
            align_embs.shape[0], 1, 1
        ).to(dtype=align_embs.dtype, device=align_embs.device)
        ...
        if self.use_future_depth:
            ...
            future_depth_preds = self.future_depth_align_head(
                future_align_embs, future_depth_preds).contiguous().float()
