...
from loongforge.embodied.model.lingbot_vla_v2.vendor.ops.robby_moe import robby_moe_forward
...
class Qwen2FusedExperts(nn.Module):
    ...
    def forward(self, module, num_experts, routing_weights, selected_experts, hidden_states):
        ...
        """
        from loongforge.embodied.model.lingbot_vla_v2.vendor.ops.fused_moe import fused_moe_forward
...
class Qwen2TokenMoeBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Host-side mirror of "last_tokens_per_expert holds a real snapshot".
        # The monitoring path used to decide this with ``counts.sum() == 0``, which
        # is a Python branch on a CUDA tensor and therefore a device sync per MoE
        # layer per step (36 layers here). The load-balance hook sets this flag
        # when it copies the snapshot in, so the same decision costs nothing.
        self.last_tokens_per_expert_valid = False
        self.register_buffer(
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self._moe_implementation == 'fused':
            ...
            weights = (expert_mask * routing_weights.unsqueeze(-1).float()
                       ).sum(dim=1).to(hidden_states.dtype)  # (B*T, num_experts)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if compute_kqv:
            ...
            raise ValueError(
                f"Invaild Operation compute_kqv={compute_kqv} and "
                f"output_atten={output_atten} with Qwen2DecoderLayer in LingBot-VLA"
            )
