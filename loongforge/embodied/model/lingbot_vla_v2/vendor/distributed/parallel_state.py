...
class ParallelState:
    ...
    allow_singleton_fsdp: bool = False
    ...
    def __post_init__(self):
        ...
        if self.dp_replicate_size * self.dp_shard_size != self.dp_size:
            raise ValueError(
                f"The product of dp_replicate_size: {self.dp_replicate_size} and "
                f"dp_shard_size: {self.dp_shard_size} should be equal to "
    ...
    def fsdp_enabled(self) -> bool:
        """Vendor interface."""
        return self.fsdp_size > 1 or self.allow_singleton_fsdp
...
def init_parallel_state(
    ...
    logger.info_rank0(
        f"Initializing parallel state... dp_size {dp_size}, "
        f"dp_replicate_size {dp_replicate_size}, "
        f"dp_shard_size {dp_shard_size}, tp_size {tp_size}, "
    ...
    if is_torch_version_greater_than("2.4"):
        ...
        if ep_size > 1:
            ...
            try:
                ...
                # torch 2.8 DeviceMesh._init_process_groups() goes through
                # split_group(), which asserts on this cluster's compound
                # "CPU:GLOO,CUDA:NCCL" default backend. Rebuild the identical
                # mesh from explicitly created process groups instead.
                logger.warning_rank0(
                    f"DeviceMesh init for EP failed ({exc}); rebuilding via from_group."
                )
                mesh_t = mesh if isinstance(mesh, torch.Tensor) else torch.tensor(mesh)
                cur_rank = dist.get_rank()
                # Every rank must call new_group for every subgroup, in the same order.
                ep_groups = {}
                for _j in range(mesh_t.size(1)):
                    _r = [int(x) for x in mesh_t[:, _j].tolist()]
                    ep_groups[tuple(_r)] = dist.new_group(ranks=_r)
                epf_groups = {}
                for _i in range(mesh_t.size(0)):
                    _r = [int(x) for x in mesh_t[_i, :].tolist()]
                    epf_groups[tuple(_r)] = dist.new_group(ranks=_r)
                my_ep, my_epf = None, None
                for _i in range(mesh_t.size(0)):
                    for _j in range(mesh_t.size(1)):
                        if int(mesh_t[_i, _j]) == cur_rank:
                            my_ep = tuple(int(x) for x in mesh_t[:, _j].tolist())
                            my_epf = tuple(int(x) for x in mesh_t[_i, :].tolist())
                ep_fsdp_device_mesh = DeviceMesh.from_group(
                    [ep_groups[my_ep], epf_groups[my_epf]],
                    device_type,
                    mesh_t,
                    mesh_dim_names=("ep", "ep_fsdp"),
                )
