...
from .vla_data import MultiVLADataset, VLADataset
...
try:
    ...
    # datasets < 4.0 has no List feature type, so saved schemas that use it must fall
    # back to the legacy Sequence. datasets >= 4.0 handles List natively and rewriting
    # it there turns list-of-struct columns into struct-of-lists, which then fails
    # Features.reorder_fields_as against the real parquet schema.
    if not hasattr(features, "List"):
...
def build_vla_dataset(
    ...
    if model_config is None:
        assert processor is None, "processor and model_config can only be None when computing norm"
