...
class FeatureTransform:
    def __init__(
        ...
        # Resolve a relative norm_stats path against the robot-config repo root
        # (two levels above the robot config file), falling back to CWD.
        if norm_stats_path and not os.path.isabs(norm_stats_path) and not os.path.exists(norm_stats_path):
            _cfg_repo_root = os.path.abspath(os.path.join(os.path.dirname(robot_config_path), '..', '..'))
            _candidate = os.path.join(_cfg_repo_root, norm_stats_path)
            if os.path.exists(_candidate):
                norm_stats_path = _candidate
            ...
            for feature_convert_info in features_convert_info:
                ...
                if feature_category == 'actions':
                    ...
                    if action_feature not in self.feature_config.joints:
                        raise ValueError(
                            f"{action_feature} in the robot config is not included among the "
                            "predefined features in the training config: "
                            f"{self.feature_config.joints}"
                        )
                    ...
                    if state_feature not in self.feature_config.joints:
                        raise ValueError(
                            f"{state_feature} in the robot config is not included among the "
                            "predefined features in the training config: "
                            f"{self.feature_config.joints}"
                        )
                    ...
                    if image_feature not in self.feature_config.images:
                        raise ValueError(
                            f"{image_feature} in the robot config is not included among the "
                            "predefined features in the training config: "
                            f"{self.feature_config.images}"
                        )
    ...
    def get_feature_mapping(self, robot_config, target_features, org_features):
            ...
            for feature_convert_info in features_convert_info:
                ...
                if isinstance(feature_convert_info, str):
                    ...
                    if feature_category == 'actions':
                        ...
                        action_subtract_state[target_feature] = feature_convert_info[
                            target_feature
                        ].pop('subtract_state')
                        ...
                        action_relative_type[target_feature] = feature_convert_info[
                            target_feature
                        ].pop('relative_type', 'quaternion_local')
                    ...
                    if 'origin_keys' in feature_convert_info[target_feature]:
                        ...
                        if isinstance(feature_convert_info[target_feature]['origin_keys'], str):
                            ...
                            origin_keys = feature_convert_info[target_feature]['origin_keys']
                            reverse_convert_features[origin_keys] = {
        ...
        if len(actions_convert_from_state) > 0:
            for action in list(actions_convert_from_state):
                ...
                state_feature = action.replace("action.", "observation.state.")
                assert state_feature in target_features['states'], (
                    f"{action} can not converted from {state_feature}"
                )
            ...
            if isinstance(convert_info, list):
                ...
                for _convert_info in convert_info:
                    if _convert_info['target_key'] not in item:
                        raise ValueError(
                            f"{_convert_info['target_key']} is not contained in robot config "
                            "as target feature"
                        )
                    concat_list.append(
    ...
    def apply(self, item, policy_eval=False):
        ...
        assert self.model_config.max_state_dim >= state_joint_mask.shape[-1], (
            "max_action_dim is smaller than the state joint dimension: "
            f"{self.model_config.max_action_dim} < {state_joint_mask.shape[-1]}"
        )
        state_joint_mask = F.pad(
            state_joint_mask,
            (0, self.model_config.max_state_dim - state_joint_mask.shape[-1]),
        ).to(dtype=torch.bool)

        action_joint_mask = batch_dict['action_joint_mask']
        assert self.model_config.max_action_dim >= action_joint_mask.shape[-1], (
            "max_action_dim is smaller than the action joint dimension: "
            f"{self.model_config.max_action_dim} < {action_joint_mask.shape[-1]}"
        )
        action_joint_mask = F.pad(
            action_joint_mask,
            (0, self.model_config.max_action_dim - action_joint_mask.shape[-1]),
        ).to(dtype=torch.bool)

        chunk_joint_mask = batch_dict['chunk_joint_mask']
        assert self.model_config.max_action_dim >= chunk_joint_mask.shape[-1], (
            "max_action_dim is smaller than the action joint dimension: "
            f"{self.model_config.max_action_dim} < {chunk_joint_mask.shape[-1]}"
        )
        chunk_joint_mask = F.pad(
            chunk_joint_mask,
            (0, self.model_config.max_action_dim - chunk_joint_mask.shape[-1]),
        ).to(dtype=torch.bool)
        ...
        for k in self.feature_config.joints:
            ...
            if action_key in self.actions and w_action:
                ...
                assert action_key in self.normalizer.norm_stats, (
                    f"{action_key} not in norm keys: "
                    f"{self.normalizer.norm_stats.keys()}"
                )
