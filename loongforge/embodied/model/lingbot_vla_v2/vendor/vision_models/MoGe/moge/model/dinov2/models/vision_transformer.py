...
class DinoVisionTransformer(nn.Module):
    ...
    def interpolate_pos_encoding(self, x, h, w):
        ...
        if not self.onnx_compatible_mode and self.interpolate_offset > 0:
            # Historical kludge: add a small number to avoid floating point
            # error in the interpolation, see
        ...
        return torch.cat((class_pos_embed[:, None, :].expand(
            patch_pos_embed.shape[0], -1, -1), patch_pos_embed), dim=1).to(previous_dtype)
