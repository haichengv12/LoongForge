...
class LingbotVlaV2BatchEnricher:
    ...
    def align_params(self) -> dict:
        ...
        return self.model_cfg.resolved_align_params()
    ...
    def setup(self, ctx=None, model_cfg=None, training_args=None) -> None:
        ...
        build_depth_model = resolve_build_depth_model(self.model_cfg)
        build_video_model = resolve_build_video_model(self.model_cfg)
    ...
    def _compute_targets(self, inputs: dict) -> dict:
        ...
        get_depth_target = resolve_get_depth_target(self.model_cfg)
        get_video_target = resolve_get_video_target(self.model_cfg)
    ...
    # -- AsyncTargetProvider --------------------------------------------------
    #
    # The public ``BoundedAsyncPipeline`` drives the look-ahead through these three
    # methods; everything LingBot-specific about the teachers stays above.

    def submit(self, batch):
        """Start the teachers for ``batch``, or compute them inline.
        ...
        nothing to compute.
        """
        handle = self.enrich_async(batch)
        if handle is None:
            self.enrich(batch)
        return handle

    def resolve(self, handle):
        """Block for a submitted handle's teacher outputs."""
