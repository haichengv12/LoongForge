...
def _muon_momentum_update(buf: Tensor, grad: Tensor, momentum: float, nesterov: bool) -> Tensor:
    """Advance the momentum buffer in place and return the Muon pre-NS update.

    Identical to the upstream Muon momentum step and shared by all three call
    sites (per-param, local mega-batch, FSDP mega-batch launch):
    ``buf <- lerp(buf, grad, 1 - momentum)``; the returned update is
    ``lerp(grad, buf, momentum)`` under Nesterov, else a clone of ``buf``.
    """
    buf.lerp_(grad, 1 - momentum)
    if nesterov:
        return grad.lerp(buf, momentum)
    return buf.clone()


def _is_adamw_by_name(name: str, extra_patterns: Sequence[str]) -> bool:
    """Return True when ``name`` matches a default or caller-supplied AdamW pattern."""
    lname = name.lower()
...
def _classify_param(p: Tensor) -> str:
    ...
    Fused MoE experts are a single 3D ``[num_experts, in, out]`` tensor, so a 3D
    path is required in addition to the 2D matmul path.
    """
...
@dataclass
class _MegabatchState:
    """In-flight state for one same-shape FSDP2 chunk, passed launch -> finish.

    ``gather_list`` / ``stacked_local`` are cleared (set to ``None``) once the
    all-gather has been consumed so their buffers free promptly.
    """

    handle: Any
    gather_list: Optional[List[Tensor]]
    stacked_local: Optional[Tensor]
    params: List[Tensor]
    has_grad: List[bool]
    config: dict
    world_size: int
...
class DistributedMuon(Optimizer):
    ...
    def step(self, closure=None):  # type: ignore[override]
        """Run one Muon step: mega-batched FSDP2 params first, then the rest."""
        loss = None
        ...
        for group in self.param_groups:
            ...
            for p, kind in other_params:
                ...
                if ortho_local is None:
                    ...
                    update_local = _muon_momentum_update(buf_local, grad_local, momentum, nesterov)
    ...
    def _prepare_local_megabatches(
        ...
        for p, _kind in params_with_kinds:
            ...
            buf = state["momentum_buffer"]
            updates[id(p)] = _muon_momentum_update(buf, p.grad, momentum, nesterov)
    ...
    def _megabatch_launch(self, params: List[Tensor], config: dict) -> Optional["_MegabatchState"]:
    ...
    def _megabatch_finish(self, st: Optional["_MegabatchState"]) -> None:
