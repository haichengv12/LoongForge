...
upstream benchmark dataset stack so the sample pipeline is numerically
...
@dataclass
class _UpstreamDatasetConfig:
    """Attribute-compatible shim for the upstream ``build_vla_dataset``."""
...
def _to_list(value):
    """OmegaConf ListConfig → plain list (upstream code uses list semantics)."""
    return list(value) if value is not None else []


def build_lingbot_vla_v2_dataset(model_cfg, data_cfg, training_args):
    """Build the upstream MultiVLADataset from typed LoongForge configs."""
    ...
    # Arm spawn DataLoader workers to install the Transformers-5.x compat at
    # their interpreter startup (they re-import upstream ``lingbotvla`` when
    # unpickling, bypassing the main-process adapter seam). Must run in the main
    # process before the DataLoader spawns any worker.
    arm_spawn_workers()

    build_vla_dataset = resolve_build_vla_dataset()
    ...
    upstream_data_cfg = _UpstreamDatasetConfig(
    ...
    dataset = build_vla_dataset(
        dataset_config=upstream_data_cfg,
